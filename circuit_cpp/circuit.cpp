...
void DoubleHashedLeaf_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void DoubleHashedLeaf_2_run(uint ctx_index,Circom_CalcWit* ctx);
void MerkleTreeRoot_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MerkleTreeRoot_3_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[4] = { 
MiMCFeistel_0_run,
MiMCSponge_1_run,
DoubleHashedLeaf_2_run,
MerkleTreeRoot_3_run };
uint get_main_input_signal_start() {return 2;}

uint get_main_input_signal_no() {return 8;}

uint get_total_signal_no() {return 12421;}

uint get_number_of_components() {return 29;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 9249;}
...
ctx->componentMemory[coffset].templateName = "DoubleHashedLeaf";
ctx->componentMemory[coffset].signalStart = soffset;
...
ctx->componentMemory[coffset].templateName = "MerkleTreeRoot";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 8;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[7];
}
...
for (uint i = 0; i < 7; i++) {
std::string new_cmp_name = "H"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
DoubleHashedLeaf_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[15]); // line circom 26
while(Fr_isTrue(&expaux[0])){
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[15]); // line circom 29
if(Fr_isTrue(&expaux[0])){
{
...
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
DoubleHashedLeaf_2_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_sub(&expaux[1],&lvar[1],&circuitConstants[17]); // line circom 33
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&expaux[1])) + 0)]].signalStart + 0]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
DoubleHashedLeaf_2_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_sub(&expaux[1],&lvar[1],&circuitConstants[17]); // line circom 34
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * (Fr_toInt(&expaux[1]) + 1)) + 0)]].signalStart + 0]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
DoubleHashedLeaf_2_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[5]); // line circom 37
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[2];
// load src
Fr_add(&expaux[0],&lvar[2],&circuitConstants[3]); // line circom 26
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[15]); // line circom 26
...
MerkleTreeRoot_3_create(1,0,ctx,"main",0);
MerkleTreeRoot_3_run(0,ctx);
}
